...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    if (bgt927) {
    	send_cfg_buf[0] = gtp_dat_gt927;
		cfg_info_len[0] =  CFG_GROUP_LEN(gtp_dat_gt927);
    }
...
static s8 gtp_request_irq(struct goodix_ts_data *ts)
{
    ...
    if (ts->irq)
    {
        ...
        ret = devm_request_threaded_irq(&(ts->client->dev), ts->irq, NULL, 
            goodix_ts_irq_handler, irq_flags | IRQF_ONESHOT /*irq_table[ts->int_trigger_type]*/, 
            ts->client->name, ts);
        if (ret != 0) {
