...
static int rockchip_sound_probe(struct platform_device *pdev)
{
	...
	cdndp_dailink.platforms->of_node = cpu_node;
	cdndp_dailink.cpus->of_node = cpu_node;

	cdndp_dailink.codecs->of_node = of_parse_phandle(pdev->dev.of_node,
							   "rockchip,codec", 0);
	if (!cdndp_dailink.codecs->of_node) {
